...
def checkout_page(request):
    ...
    for item in cart_items:
        ...
        offer_discount += item.offer_data["discount_amount"] * item.quantity
...
def place_order(request):
    ...
    address = Address.objects.filter(id=selected_address_id, user=request.user).first()
    ...
    for item in cart_items:
        ...
        offer_discount += offer_data["discount_amount"] * item.quantity
    ...
    with transaction.atomic():
        ...
        if request.user.referred_by and not request.user.referral_reward_given:
            ...
            WalletTransaction.objects.create(
                ...
                status="Completed",
                amount=Decimal("200"),
                description=(f"Referral Reward - " f"{request.user.email}"),
            )
        ...
        for item in cart_items:
            ...
            item_total = offer_data["final_price"] * item.quantity

            discount_share = Decimal("0")

            if discount > 0 and subtotal > 0:

                discount_share = (item_total / subtotal) * discount
    ...
    with transaction.atomic():
        ...
        for item in cart_items:
            ...
            if discount > 0:
                ...
                discount_share = (item_total / total_cart_value) * discount
...
def razorpay_success(request, order_id):
    ...
    order = Order.objects.filter(
        order_id=order_id, user=request.user, payment_method="RAZORPAY"
    ).first()

    if not order:

        messages.error(request, "Razorpay order not found")
...
def order_detail(request, order_id):
    ...
    active_delivered_items = order.items.exclude(
    ...
    return_requested_exists = order.items.filter(
        item_status__in=[
            "Return Requested",
            "Return Approved",
            "Return Rejected"
    ...
    for item in order.items.all():
        ...
        offer_discount += original_total - item.total_price
    ...
    context = {
        ...
        "offer_discount": offer_discount,
        "active_delivered_items": active_delivered_items,
        "active_items": active_items,
        "cancelled_items": cancelled_items,
        "return_requested_exists": return_requested_exists,
    }
...
def cancel_order(request, order_id):
    ...
    if request.method == "POST":
        ...
        return redirect("cancel_success", order.order_id)
...
def cancel_success(request, order_id):
    ...
    return render(request, "cancel_success.html", context)
...
def cancel_order_item(request, item_id):
    ...
    order_item = (
        ...
        .filter(id=item_id, order__user=request.user)
    ...
    if order.payment_method in ["RAZORPAY", "WALLET"]:
        ...
        refund_amount = order_item.total_price - order_item.discount_share
        # ================= DUPLICATE REFUND PROTECTION =================
    ...
    for item in active_items:
        ...
        remaining_total += item.total_price - item.discount_share
...
def return_order(request, order_id):
    ...
    if request.method == "POST":
        ...
        additional_reason = request.POST.get("additional_reason", "").strip()
        ...
        if not selected_items:
            ...
            return redirect("return_order", order.order_id)
    ...
    context = {"order": order, "returnable_items": returnable_items,}
...
def invoice_page(request, order_id):
    ...
    invoice_items = order.items.exclude(
        item_status__in=[
            "Cancelled",
            "Return Approved"
        ]
