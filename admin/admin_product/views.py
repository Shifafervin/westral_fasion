...
def variant_management(request, product_id):
    ...
    all_variants = product.variants.filter(
    is_deleted=False
    )

    total_variants = all_variants.count()

    active_variants = all_variants.filter(
        is_active=True
    ).count()

    out_of_stock = all_variants.filter(
        stock=0
    ).count()


    default_variant = all_variants.filter(
            is_default=True
    ).first()
