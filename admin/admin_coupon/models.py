...
class Coupon(models.Model):
    ...
    def clean(self):
        ...
        if not re.match(r"^(?=.*[A-Z])[A-Z0-9]+$", self.code.upper()):
            raise ValidationError(
                {
                    "code": "Coupon code must contain only uppercase letters and numbers. No spaces, underscores or special characters are allowed."
                }
            )
        ...
        if self.discount_type == self.PERCENTAGE:
            ...
            if self.discount_value <= 0:
                ...
                raise ValidationError(
                    {"discount_value": "Percentage discount must be greater than 0."}
                )
        ...
        if self.usage_limit_per_user <= 0:
            ...
            raise ValidationError(
                {"usage_limit_per_user": "User usage limit must be greater than zero."}
            )
